...
export default {
  ...
  methods: {
    async shareArticle() {
      const articleInfo = { title: this.article.title, url: this.article.url};
      if(navigator.share) {
        try {
...
<template>
  <pv-card class="m-2">
    ...
    <template #subtitle>
      ...
      <p class="flex align-content-start flex-wrap">
        <span class="flex align-items-center justify-content-center text-sm mr-2">
          By {{ article.author }}
        </span>
        <span class="flex align-items-center justify-content-center text-sm">
          {{ article.getFullFormattedPublishedAt() }}
        </span>
      </p>
    </template>
    <template #content>
